...
annual_target = parse_revenue_input(annual_target_input)
monthly_target = annual_target/12 if annual_target else 0
rev_per_ad = parse_revenue_input(rev_per_ad_input)
...
for i in range(num_products):
    with st.expander(f"Lineup {i+1}", expanded=True):
        ...
        st.session_state.setdefault(mon_key, format_int(default_ann/12))
        st.session_state.setdefault(day_key, format_int(default_ann/365))
        st.session_state.setdefault(hr_key, 15)

        primary = c1.selectbox(
            "Primary Product", available_products,
            index=available_products.index(st.session_state[prim_key]),
            key=prim_key
        )
        complementary = c2.selectbox(
            "Complementary Product", available_products,
            index=available_products.index(st.session_state[comp_key]),
            key=comp_key
        )
        ...
        r1.text_input("Annual Revenue (€)", format_int(ann_val),
                      key=rev_key, on_change=annual_changed, args=(i,))
        r2.text_input("Monthly Revenue (€)", format_int(mon_val),
                      key=mon_key, on_change=monthly_changed, args=(i,))
        r3.text_input("Daily Revenue (€)", format_int(day_val),
                      key=day_key, on_change=daily_changed, args=(i,))

        hit_rate = st.number_input("Hit Rate (%)", 0, 100,
                                   value=int(st.session_state[hr_key]),
                                   step=1, format="%d", key=hr_key)

        product_lineups.append({
            'primary': primary,
...
for ln in product_lineups:
    ...
    actual = (
        b1*bundle_template['bundle1']['price'] +
        b2*bundle_template['bundle2']['price'] +
        b3*bundle_template['bundle3']['price']
    )
    prim_qty = (
        b1*bundle_template['bundle1']['composition']['primary'] +
        b2*bundle_template['bundle2']['composition']['primary'] +
        b3*bundle_template['bundle3']['composition']['primary']
    )
    comp_qty = (
        b1*bundle_template['bundle1']['composition']['complementary'] +
    ...
    lineup_results.append({
        **ln,
        ...
        'bundle_qty':      {'B1':b1,'B2':b2,'B3':b3},
        'actual_rev':      actual,
        'prod_qty':        prod_qty,
        'total_bundles':   tb,
        'total_units':     sum(prod_qty.values())
    })

total_actual = sum(r['actual_rev'] for r in lineup_results)
...
for r in lineup_results:
    for p,q in r['prod_qty'].items():
        combined_prod[p] = combined_prod.get(p,0) + q
total_units  = sum(combined_prod.values())

# ---- ALLOCATION SUMMARY ----
alloc_pct = total_alloc/annual_target if annual_target else 0
st.subheader("Allocation Summary")
cA,cB,cC = st.columns(3)
cA.metric("Annual Target", format_currency(annual_target), delta=format_currency(total_alloc-annual_target))
cB.metric("Monthly Forecast", format_currency(total_alloc/12), delta=format_currency((total_alloc/12)-monthly_target))
cC.metric("Allocation %", f"{int(round(alloc_pct*100))}%")
st.progress(min(alloc_pct,1.0))
...
with tab1:
    ...
    df_ads = pd.DataFrame([{
        'Product': f"{r['primary']} + {r['complementary']}",
        'Monthly Revenue': r['monthly_revenue'],
        'Hit Rate (%)': r['hit_rate'],
        'Winners Needed': np.ceil(r['monthly_revenue']/rev_per_ad),
        'Ad Sets to Launch': np.ceil((r['monthly_revenue']/rev_per_ad)/(r['hit_rate']/100))
    } for r in lineup_results])
    totals = {
        'Product':'TOTAL',
        'Monthly Revenue': df_ads['Monthly Revenue'].sum(),
        'Hit Rate (%)':'',
        'Winners Needed': df_ads['Winners Needed'].sum(),
        'Ad Sets to Launch': df_ads['Ad Sets to Launch'].sum()
    }
    df_ads = pd.concat([df_ads, pd.DataFrame([totals])], ignore_index=True)
    st.dataframe(df_ads.style.format({
        'Monthly Revenue': lambda v: format_currency(v),
        'Winners Needed':   lambda v: format_int(v),
        'Ad Sets to Launch':lambda v: format_int(v),
        'Hit Rate (%)':     lambda v: f"{int(v)}%" if v!='' else ''
    }), use_container_width=True)

with tab2:
    m1,m2,m3 = st.columns(3)
    m1.metric("Monthly Revenue", format_currency(total_actual), f"Annual: {format_currency(total_actual*12)}")
    m2.metric("Total Bundles", format_int(total_b1+total_b2+total_b3),
              f"B1:{format_int(total_b1)} | B2:{format_int(total_b2)} | B3:{format_int(total_b3)}")
    m3.metric("Total Units", format_int(total_units))
    pie_df = pd.DataFrame({'Bundle':['B1','B2','B3'],'Count':[total_b1,total_b2,total_b3]})
    pie = alt.Chart(pie_df).mark_arc(innerRadius=50).encode(
        theta='Count:Q', color='Bundle:N', tooltip=['Bundle','Count']
    )
    ...
    st.table(df_req.assign(**{
        'Monthly Units': df_req['Monthly Units'].map(format_int),
        'Annual Units':  df_req['Annual Units'].map(format_int)
    }))
...
with tab3:
    for idx, r in enumerate(lineup_results, start=1):
        ...
        fmt_map = {
            'Qty':            lambda v: format_int(v),
            'Price':          lambda v: format_currency(v) if v!='' else '',
            'Revenue':        lambda v: format_currency(v),
            '% of Revenue':   lambda v: f"{v:.1f}%" if v!='' else ''
        }
        fmt_map[f"{prim_name} Units"] = lambda v: format_int(v)
        if not same:
            fmt_map[f"{comp_name} Units"] = lambda v: format_int(v)

        st.dataframe(
            df_rev.style.format(fmt_map),
            use_container_width=True
        )
...
info_df = pd.DataFrame([
    {
        'Bundle': bundle_template[k]['name'],
        'Price':  bundle_template[k]['price'],
        'Composition': f"{bundle_template[k]['composition']['primary']}P, {bundle_template[k]['composition']['complementary']}C",
        'Distribution (%)': bundle_distribution[k]
    }
    for k in bundle_template
])
st.table(info_df.assign(**{
    'Price':            info_df['Price'].map(format_currency),
    'Distribution (%)': info_df['Distribution (%)'].map(lambda x: f"{x:.1f}%")
}))
